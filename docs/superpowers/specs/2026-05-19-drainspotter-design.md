...
- **Demo-CSV bundled:** Button „Beispieldaten laden" im Empty-State lädt eine vorbereitete CSV unter `/public/demo.csv`. Inhalt: User-bereitgestelltes Real-Sample (15 Tage, 1.–15.04.2026, alle Modelle, ~120 User), Usernames anonymisiert nach folgendem Schema:
    - **Top-Drainer** (höchster Summen-`aic_gross_amount`): bekommen aristokratisch-villainöse Vielfrass-Namen. **`joe-gastown` ist explizit der Top-1-Drainer** (wird auf den Real-User mit dem höchsten Summen-Spend gemappt). Plätze 2–10 in absteigender Spend-Reihenfolge: `chief-token-officer`, `lord-of-the-pool`, `count-contextula`, `sir-burns-a-lot`, `duke-of-drain`, `promptzilla`, `token-shredder`, `quota-muncher`, `voldetoken`
